...
public class AddItemActivity extends AppCompatActivity {
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        ArrayAdapter<Style> styleAdapter = new ArrayAdapter<>(this, android.R.layout.simple_spinner_item, Style.values());
        ...
        buttonAddColor.setOnClickListener(v -> {
            ...
            for (int i = 0; i < ClothingColor.values().length; i++) {
                checked[i] = selectedColors.contains(colorNames[i]);
            }
            new AlertDialog.Builder(this)
                    .setTitle("Select Colors")
                    .setMultiChoiceItems(colorNames, checked, (dialog, which, isChecked) -> {
                        if (isChecked) selectedColors.add(colorNames[which]);
                        else selectedColors.remove(colorNames[which]);
                    })
